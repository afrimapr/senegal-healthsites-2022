...
#TODO create simple names for osm data
dfosm2 <- dfosm2 %>% 
...
dfosm_matches_dprs_fullname <- dfosm %>% 
...
This leaves **257** facilities in the OSM survey data that do not match names in the DPRS data. This can be because :

1. facility is not present in DPRS (note the survey included pharmacies not present in DPRS)
1. the simplified name still differs from how the same facility is named within the OSM data.

The 257 un-matched names are shown in the table below.
